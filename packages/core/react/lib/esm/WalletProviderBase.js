...
export function WalletProviderBase({ adapter, children, isUnloadingRef, onAutoConnectRequest, onConnectError, onError, onSelectWallet, wallets: adapters, }) {
    ...
    const handleError = useCallback((error, adapter) => {
    ...
    const handleConnect = useCallback(async () => {
        if (isConnecting.current || isDisconnecting.current || wallet?.adapter.connected)
            return;
        if (!wallet)
            throw handleError(new WalletNotSelectedError());
